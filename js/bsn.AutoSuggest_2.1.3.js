...
_b.AutoSuggest.prototype.getSuggestions = function(val) {
	...
	if (this.nInpC > ol && l && l < this.oP.maxentries && this.oP.cache) {
		var arr = [];
		for (var i = 0; i < l; i++) {
			if (this.aSug[i].value.substr(0, val.length).toLowerCase() == val
					.toLowerCase()
					|| this.aSug[i].info == 'as_header')
...
_b.AutoSuggest.prototype.doAjaxRequest = function(input) {
	...
	if (typeof(this.oP.script) == "function")
		...
		var url = this.oP.script + this.oP.varname + "="
...
_b.AutoSuggest.prototype.createList = function(arr) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		var output = val.substring(0, st) + "<font style='font-weight:bold'>"
				+ val.substring(st, st + this.sInp.length) + "</font>"
...
_b.AutoSuggest.prototype.clearSuggestions = function(fade) {
	...
	var pointer = this;
	if (ele)
		fade ? new _b.Fader(ele, 1, 0, 250, function() {
...
_b.Fader.prototype._fade = function() {
	...
	var ieop = Math.round(this
			._tween(this.nTime, this.from, this.to, this.nDur)
			* 100);
	var op = ieop / 100;
	...
	if (this.e.filters) // internet explorer
	{
		try {
			...
			this.e.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity='
					+ ieop + ')';
		}
	} else // other browsers
	{
