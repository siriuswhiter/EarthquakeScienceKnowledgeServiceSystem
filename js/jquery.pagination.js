...
(function($) {
	...
	$.extend($.PaginationCalculator.prototype, {
				...
				numPages : function() {
					return Math
							.ceil(this.maxentries / this.opts.items_per_page);
				},
				/**
				...
				getInterval : function(current_page) {
					...
					var start = current_page > ne_half
							? Math.max(Math.min(current_page - ne_half,
											upper_limit), 0)
							: 0;
					var end = current_page > ne_half
							? Math
									.min(
											current_page
													+ ne_half
	...
	$.extend($.PaginationRenderers.defaultRenderer.prototype, {
		...
		getLinks : function(current_page, eventHandler) {
			var begin, end, interval = this.pc.getInterval(current_page), np = this.pc
			...
			if (interval.start > 0 && this.opts.num_edge_entries > 0) {
				...
				if (this.opts.num_edge_entries < interval.start
					...
					jQuery("<span>" + this.opts.ellipse_text + "</span>")
							.appendTo(fragment);
				}
			}
